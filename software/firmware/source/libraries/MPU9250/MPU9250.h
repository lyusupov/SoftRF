...
public:
    ...
    void sleep(bool b) {
        byte c = read_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_1);  // read the value, change sleep bit to match b, write byte back to register
        if (b) {
            c = c | 0x40;  // sets the sleep bit
        } else {
            c = c & 0xBF;  // mask 1011111 keeps all the previous bits
        }
        write_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_1, c);
    }
    ...
    bool isSleeping() {
        byte c = read_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_1);
        return (c & 0x40) == 0x40;
    }

    bool available() {
        return has_connected && (read_byte(mpu_i2c_addr, MPU9250_INT_STATUS) & 0x01);
    }
...
private:
    void initMPU9250() {
        ...
        // reset device
        write_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_1, 0x80);  // Write a one to bit 7 reset bit; toggle reset device
        delay(100);

        // wake up device
        write_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_1, 0x00);  // Clear sleep mode bit (6), enable all sensors
        delay(100);                                  // Wait for all registers to reset

        // get stable time source
        write_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_1, 0x01);  // Auto select clock source to be PLL gyroscope reference if ready else
        delay(200);
        ...
        write_byte(mpu_i2c_addr, MPU9250_MPU_CONFIG, mpu_config);

        // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
        uint8_t sample_rate = (uint8_t)setting.fifo_sample_rate;
        write_byte(mpu_i2c_addr, MPU9250_SMPLRT_DIV, sample_rate);  // Use a 200 Hz rate; a rate consistent with the filter update rate
                                                            // determined inset in CONFIG above

        // Set gyroscope full scale range
        // Range selects FS_SEL and GFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
        uint8_t c = read_byte(mpu_i2c_addr, MPU9250_GYRO_CONFIG);  // get current GYRO_CONFIG register value
        ...
        write_byte(mpu_i2c_addr, MPU9250_GYRO_CONFIG, c);          // Write new GYRO_CONFIG value to register

        // Set accelerometer full-scale range configuration
        c = read_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG);     // get current ACCEL_CONFIG register value
        c = c & ~0xE0;                                 // Clear self-test bits [7:5]
        c = c & ~0x18;                                 // Clear ACCEL_FS_SEL bits [4:3]
        c = c | (uint8_t(setting.accel_fs_sel) << 3);  // Set full scale range for the accelerometer
        write_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG, c);     // Write new ACCEL_CONFIG register value

        // Set accelerometer sample rate configuration
        // It is possible to get a 4 kHz sample rate from the accelerometer by choosing 1 for
        // accel_fchoice_b bit [3]; in this case the bandwidth is 1.13 kHz
        c = read_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG2);        // get current ACCEL_CONFIG2 register value
        ...
        write_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG2, c);        // Write new ACCEL_CONFIG2 register value
        ...
        write_byte(mpu_i2c_addr, MPU9250_INT_PIN_CFG, 0x22);
        write_byte(mpu_i2c_addr, MPU9250_INT_ENABLE, 0x01);  // Enable data ready (bit 0) interrupt
        delay(100);
    }
...
private:
    void read_accel_gyro(int16_t* destination) {
        ...
        read_bytes(mpu_i2c_addr, MPU9250_ACCEL_XOUT_H, 14, &raw_data[0]);             // Read the 14 raw data registers into data array
...
private:
    ...
    int16_t read_temperature_data() {
        ...
        read_bytes(mpu_i2c_addr, MPU9250_TEMP_OUT_H, 2, &raw_data[0]);  // Read the two raw data registers sequentially into data array
    ...
    void set_acc_gyro_to_calibration() {
        ...
        write_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_1, 0x01);
        write_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_2, 0x00);
        delay(200);

        // Configure device for bias calculation
        write_byte(mpu_i2c_addr, MPU9250_INT_ENABLE, 0x00);    // Disable all interrupts
        write_byte(mpu_i2c_addr, MPU9250_FIFO_EN, 0x00);       // Disable FIFO
        write_byte(mpu_i2c_addr, MPU9250_PWR_MGMT_1, 0x00);    // Turn on internal clock source
        write_byte(mpu_i2c_addr, MPU9250_I2C_MST_CTRL, 0x00);  // Disable I2C master
        write_byte(mpu_i2c_addr, MPU9250_USER_CTRL, 0x00);     // Disable FIFO and I2C master modes
        write_byte(mpu_i2c_addr, MPU9250_USER_CTRL, 0x0C);     // Reset FIFO and DMP
        delay(15);

        // Configure MPU6050 gyro and accelerometer for bias calculation
        write_byte(mpu_i2c_addr, MPU9250_MPU_CONFIG, 0x01);    // Set low-pass filter to 188 Hz
        write_byte(mpu_i2c_addr, MPU9250_SMPLRT_DIV, 0x00);    // Set sample rate to 1 kHz
        write_byte(mpu_i2c_addr, MPU9250_GYRO_CONFIG, 0x00);   // Set gyro full-scale to 250 degrees per second, maximum sensitivity
        write_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG, 0x00);  // Set accelerometer full-scale to 2 g, maximum sensitivity

        // Configure FIFO to capture accelerometer and gyro data for bias calculation
        write_byte(mpu_i2c_addr, MPU9250_USER_CTRL, 0x40);  // Enable FIFO
        write_byte(mpu_i2c_addr, MPU9250_FIFO_EN, 0x78);    // Enable gyro and accelerometer sensors for FIFO  (max size 512 bytes in MPU-9150)
    ...
    void collect_acc_gyro_data_to(float* a_bias, float* g_bias) {
        ...
        write_byte(mpu_i2c_addr, MPU9250_FIFO_EN, 0x00);             // Disable gyro and accelerometer sensors for FIFO
        read_bytes(mpu_i2c_addr, MPU9250_FIFO_COUNTH, 2, &data[0]);  // read FIFO sample count
        ...
        for (uint16_t ii = 0; ii < packet_count; ii++) {
            ...
            read_bytes(mpu_i2c_addr, MPU9250_FIFO_R_W, 12, &data[0]);              // read data for averaging
    ...
    void write_accel_offset() {
        ...
        read_bytes(mpu_i2c_addr, MPU9250_XA_OFFSET_H, 2, &read_data[0]);  // Read factory accelerometer trim values
        acc_bias_reg[0] = ((int16_t)read_data[0] << 8) | read_data[1];
        read_bytes(mpu_i2c_addr, MPU9250_YA_OFFSET_H, 2, &read_data[0]);
        acc_bias_reg[1] = ((int16_t)read_data[0] << 8) | read_data[1];
        read_bytes(mpu_i2c_addr, MPU9250_ZA_OFFSET_H, 2, &read_data[0]);
        ...
        write_byte(mpu_i2c_addr, MPU9250_XA_OFFSET_H, write_data[0]);
        write_byte(mpu_i2c_addr, MPU9250_XA_OFFSET_L, write_data[1]);
        write_byte(mpu_i2c_addr, MPU9250_YA_OFFSET_H, write_data[2]);
        write_byte(mpu_i2c_addr, MPU9250_YA_OFFSET_L, write_data[3]);
        write_byte(mpu_i2c_addr, MPU9250_ZA_OFFSET_H, write_data[4]);
        write_byte(mpu_i2c_addr, MPU9250_ZA_OFFSET_L, write_data[5]);
    }
    ...
    void write_gyro_offset() {
        ...
        write_byte(mpu_i2c_addr, MPU9250_XG_OFFSET_H, gyro_offset_data[0]);
        write_byte(mpu_i2c_addr, MPU9250_XG_OFFSET_L, gyro_offset_data[1]);
        write_byte(mpu_i2c_addr, MPU9250_YG_OFFSET_H, gyro_offset_data[2]);
        write_byte(mpu_i2c_addr, MPU9250_YG_OFFSET_L, gyro_offset_data[3]);
        write_byte(mpu_i2c_addr, MPU9250_ZG_OFFSET_H, gyro_offset_data[4]);
        write_byte(mpu_i2c_addr, MPU9250_ZG_OFFSET_L, gyro_offset_data[5]);
    }
    ...
    bool self_test_impl()  // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
    {
        ...
        write_byte(mpu_i2c_addr, MPU9250_SMPLRT_DIV, 0x00);       // Set gyro sample rate to 1 kHz
        write_byte(mpu_i2c_addr, MPU9250_MPU_CONFIG, 0x02);       // Set gyro sample rate to 1 kHz and DLPF to 92 Hz
        write_byte(mpu_i2c_addr, MPU9250_GYRO_CONFIG, FS << 3);   // Set full scale range for the gyro to 250 dps
        write_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG2, 0x02);    // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
        write_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG, FS << 3);  // Set full scale range for the accelerometer to 2 g

        for (int ii = 0; ii < 200; ii++) {  // get average current values of gyro and acclerometer

            read_bytes(mpu_i2c_addr, MPU9250_ACCEL_XOUT_H, 6, &raw_data[0]);          // Read the six raw data registers into data array
            ...
            read_bytes(mpu_i2c_addr, MPU9250_GYRO_XOUT_H, 6, &raw_data[0]);           // Read the six raw data registers sequentially into data array
        ...
        write_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG, 0xE0);  // Enable self test on all three axes and set accelerometer range to +/- 2 g
        write_byte(mpu_i2c_addr, MPU9250_GYRO_CONFIG, 0xE0);   // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
        ...
        write_byte(mpu_i2c_addr, MPU9250_ACCEL_CONFIG, 0x00);
        write_byte(mpu_i2c_addr, MPU9250_GYRO_CONFIG, 0x00);
        delay(25);  // Delay a while to let the device stabilize

        // Retrieve accelerometer and gyro factory Self-Test Code from USR_Reg
        uint8_t self_test_data[6];
        self_test_data[0] = read_byte(mpu_i2c_addr, MPU9250_SELF_TEST_X_ACCEL);  // X-axis accel self-test results
        self_test_data[1] = read_byte(mpu_i2c_addr, MPU9250_SELF_TEST_Y_ACCEL);  // Y-axis accel self-test results
        self_test_data[2] = read_byte(mpu_i2c_addr, MPU9250_SELF_TEST_Z_ACCEL);  // Z-axis accel self-test results
        self_test_data[3] = read_byte(mpu_i2c_addr, MPU9250_SELF_TEST_X_GYRO);   // X-axis gyro self-test results
        self_test_data[4] = read_byte(mpu_i2c_addr, MPU9250_SELF_TEST_Y_GYRO);   // Y-axis gyro self-test results
        self_test_data[5] = read_byte(mpu_i2c_addr, MPU9250_SELF_TEST_Z_GYRO);   // Z-axis gyro self-test results
