...
void gdl90_escape_message_for_tx(gdl_message_t *rawMsg, gdl_message_escaped_t *escapedMsg) {
    ...
    switch(rawMsg->messageId) {
        ...
        case(MSG_ID_LONG_REPORT):
            ...
            break;
        case(MSG_ID_FOREFLIGHT):
            originalLen = GDL90_MSG_LEN_FOREFLIGHT;
            break;
    ...
    // One frame byte and msg ID byte are outside the data[] array.
    // One frame byte and 2 byte CRC are inside data[]
    originalLen += 3;
