...
#define ARDUINO_RA4M1_RELEASE "1.2.0"
...
// #include <core_version.h> // TODO
#define ARDUINO_EFR32_RELEASE "2.1.0"
...
static void EFR32_Bluetooth_setup()
{
  BT_name += "-";
  BT_name += String(SoC->getChipId() & 0x00FFFFFFU, HEX);
...
//      BLE.setAdvertisedService(BATService);
      ...
      const char *Firmware      = "Arduino EFR32 " ARDUINO_EFR32_RELEASE;
...
static void EFR32_Bluetooth_loop()
{
  switch (settings->bluetooth)
  {
  case BLUETOOTH_LE_HM10_SERIAL:
...
static void EFR32_Bluetooth_fini()
{
  /* TBD */
...
static int EFR32_Bluetooth_available()
{
  int rval = 0;
...
static int EFR32_Bluetooth_read()
{
  int rval = -1;
...
static size_t EFR32_Bluetooth_write(const uint8_t *buffer, size_t size)
{
  size_t rval = size;
...
IODev_ops_t EFR32_Bluetooth_ops = {
  "EFR32 Bluetooth",
  EFR32_Bluetooth_setup,
  EFR32_Bluetooth_loop,
  EFR32_Bluetooth_fini,
  EFR32_Bluetooth_available,
  EFR32_Bluetooth_read,
  EFR32_Bluetooth_write
};
